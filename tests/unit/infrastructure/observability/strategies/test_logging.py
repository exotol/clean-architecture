...
@pytest.fixture
def mock_logger():
    with patch("app.infrastructure.observability.strategies.logging.loguru_logger") as mock:
        yield mock
...
def test_log_start(
    ...
    strategy = StandardLoggingStrategy()
    ...
    if expected.bind_called:
        ...
        mock_logger.bind.return_value.info.assert_called_with(expected.info_called_with, entity.event_name)
...
def test_log_success(
    ...
    if expected.bind_called:
        ...
        context.bind.return_value.info.assert_called_with(expected.info_called_with, entity.event_name)
    else:
...
def test_log_error(
    ...
    context.exception.assert_called_with(expected.exception_called_with, entity.event_name)
